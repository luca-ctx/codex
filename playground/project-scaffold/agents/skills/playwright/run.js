...
function installPlaywright() {
    console.log('Playwright not found. Installing via pnpm...');
    try {
        ...
        console.error('Please run manually: cd', __dirname, '&& pnpm install && pnpm exec playwright install');
        return false;
    }
}
...
async function main() {
    ...
    const tempDir = path.join(os.tmpdir(), 'playwright-skill');
    try {
    ...
    try {
        ...
        // Ensure scripts can resolve local node_modules when executed from /tmp
        ...
        if (!existingNodePath.includes(skillNodeModules)) {
            existingNodePath.unshift(skillNodeModules);
